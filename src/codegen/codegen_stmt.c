...
void generate_statement_code(statement *stmt) {
    ...
    switch (stmt->stmt_type) {
        ...
        case ST_VAR_DECL:
            if (current_function_generated.decl == NULL) {
        ...
        case ST_WHILE:
            ...
            generate_failing_condition_jump(stmt->expr, "wh%d_end", get_curr_while_num());
            generate_statement_code(stmt->body);
            ir_jmp("wh%d", get_curr_while_num());
