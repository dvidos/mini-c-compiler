    ...
    */
    ...
    if (executable) {
        ...
        elf64_prog_header *text_pheader = create_program_header64(mp, 
            ...
            contents->text->mem_address, bin_len(contents->text->contents));
...
static bool parse_elf64_obj_header(binary *file_data, u64 *sections_offset, u64 *sections_count, u64 *section_names_table_index) {
    ...
    bin_read_mem(file_data, h, sizeof(elf64_header));
    ...
    str *out_name, binary *out_content)
{
    ...
    bin_seek(file_data, sections_offset + section_no * sizeof(elf64_section_header));
    bin_read_mem(file_data, &section, sizeof(elf64_section_header));

    // extract the name from the names table (if supported)
    if (out_name != NULL && names_table != NULL) {
        str_clear(out_name);
        bin_seek(names_table, section.name);
        two_bytes[0] = bin_read_byte(names_table);
        two_bytes[1] = 0;
        while (two_bytes[0] != 0) {
...
static void elf_unit_test_obj_file(mempool *mp) {
    ...
    bin_add_mem(saved->text->contents, buffer, sizeof(buffer));

    memset(buffer, '-', sizeof(buffer));
    strcpy(buffer, "data contents");
    bin_add_mem(saved->data->contents, buffer, sizeof(buffer));

    memset(buffer, '-', sizeof(buffer));
    strcpy(buffer, "bss contents");
    bin_add_mem(saved->bss->contents, buffer, sizeof(buffer));

    memset(buffer, '-', sizeof(buffer));
    strcpy(buffer, "rodata contents");
    bin_add_mem(saved->rodata->contents, buffer, sizeof(buffer));

    memset(buffer, '-', sizeof(buffer));
    strcpy(buffer, "rela_text contents");
    bin_add_mem(saved->rela_text->contents, buffer, sizeof(buffer));

    memset(buffer, '-', sizeof(buffer));
    strcpy(buffer, "symtab contents");
    bin_add_mem(saved->symtab->contents, buffer, sizeof(buffer));

    memset(buffer, '-', sizeof(buffer));
    strcpy(buffer, "strtab contents");
    bin_add_mem(saved->strtab->contents, buffer, sizeof(buffer));

    memset(buffer, '-', sizeof(buffer));
    strcpy(buffer, "comment contents");
    bin_add_mem(saved->comment->contents, buffer, sizeof(buffer));
    ...
    bin_read_mem(loaded->text->contents, buffer, sizeof(buffer));
    assert(strcmp(buffer, "text contents") == 0);

    bin_read_mem(loaded->data->contents, buffer, sizeof(buffer));
    assert(strcmp(buffer, "data contents") == 0);

    bin_read_mem(loaded->rodata->contents, buffer, sizeof(buffer));
    assert(strcmp(buffer, "rodata contents") == 0);

    bin_read_mem(loaded->rela_text->contents, buffer, sizeof(buffer));
    assert(strcmp(buffer, "rela_text contents") == 0);

    bin_read_mem(loaded->symtab->contents, buffer, sizeof(buffer));
    assert(strcmp(buffer, "symtab contents") == 0);

    bin_read_mem(loaded->strtab->contents, buffer, sizeof(buffer));
    assert(strcmp(buffer, "strtab contents") == 0);

    bin_read_mem(loaded->comment->contents, buffer, sizeof(buffer));
...
static void elf_unit_test_executable_file(mempool *mp) {
    ...
    strcpy(buffer, "text contents");
    bin_add_mem(saved->text->contents, buffer, sizeof(buffer));
    bin_pad(saved->text->contents, 0, 4096);

    strcpy(buffer, "data contents");
    bin_add_mem(saved->data->contents, buffer, sizeof(buffer));
    bin_pad(saved->data->contents, 0, 4096);

    strcpy(buffer, "bss contents");
    bin_add_mem(saved->bss->contents, buffer, sizeof(buffer));
    bin_pad(saved->bss->contents, 0, 4096);
    
    strcpy(buffer, "rodata contents");
    bin_add_mem(saved->rodata->contents, buffer, sizeof(buffer));
    bin_pad(saved->rodata->contents, 0, 4096);
    ...
    bin_seek(elf_data, 0);
    assert(bin_read_byte(elf_data) == 0x7F);
    assert(bin_read_byte(elf_data) == 'E');
    assert(bin_read_byte(elf_data) == 'L');
    assert(bin_read_byte(elf_data) == 'F');
